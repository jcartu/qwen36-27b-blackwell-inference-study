...
| 15 | [`15-v17-tp1-maxctx/`](./15-v17-tp1-maxctx/) | TP=1 single-GPU at **max safe context** — how much of 262k fits on one card? | Same v17 TP=1 sweep, `--gpu-memory-utilization 0.92`, `--max-model-len 253952` (single-card safe max), `--kv-budget 255042`, `expandable_segments`. | Serves **253,952 ctx** (~97% of full 262k; 0.94 OOMs under decode load, documented). KV 255,042 tokens → **23/41 cells run**. Peak **642 tok/s @ c=64×0**; 128k prefill 3,867 tok/s now reachable. Raising mem buys capacity, not peak throughput. |
| 16 | [`16-v17-tp2-bf16-posttune/`](./16-v17-tp2-bf16-posttune/) | TP=2 BF16 re-run **after host CPU turbo fix** — quantifies the ~10% low-concurrency uplift. | Identical to Exp 13 (DFlash N=8, TP=2, 0.80, kv 854152); only change is host CPU: removed `max_cstate=2` boot cap (C6 restored) + `performance` governor. Single-core boost 2,898→3,450 MHz. | Peak **1,171 tok/s @ c=32×0** (vs 1,138). **+10-12% at c=1-4** (low concurrency, CPU-bound per-token work), ~flat at high concurrency (GPU-bound). Residual single-core 3.5GHz + uncore 2.5GHz caps are BIOS-only. |
| 17 | [`17-v17-tp2-fp8-mtp3/`](./17-v17-tp2-fp8-mtp3/) | TP=2 **FP8 W8A8 + MTP=3** (post CPU-tune) — FP8 counterpart to Exp 16. | `Qwen3.6-27B-FP8`, MTP=3, TP=2, 0.80, kv 1,711,960 (6.53× conc, ~2× BF16 budget). Same post-tune host. | Peak **2,960 tok/s @ c=128×0** — fastest config in the study. Scales to c=128 (BF16 plateaus ~c=32). FP8 vs BF16: +34% (c=1) to +166% (c=128). ~10% CPU-tune uplift at low concurrency still applies. |
| 18 | [`18-v17-tp1-bf16-posttune/`](./18-v17-tp1-bf16-posttune/) | TP=1 single-GPU BF16 (post CPU-tune) — TP=1 counterpart to Exp 16, vs pre-tune Exp 14. | `Qwen3.6-27B` BF16+DFlash N=8, **TP=1**, 0.80, max-model-len 131072, kv 134,606. Post-tune host. | Peak **444 tok/s @ c=16×0** (vs 407 pre-tune Exp 14, **+9%** CPU fix). 26/41 cells KV-skipped (single-card). ~10% CPU uplift most visible at low concurrency. |
| 19 | [`19-v17-tp1-fp8-mtp3/`](./19-v17-tp1-fp8-mtp3/) | TP=1 single-GPU **FP8 + MTP=3** (post CPU-tune) — final cell of the TP×quant matrix. | `Qwen3.6-27B-FP8`, MTP=3, **TP=1**, 0.80, **full 262k ctx fits on one card** (kv 621,350, 2.37× conc). | Peak **2,062 tok/s @ c=64×0** — single card nearly matches old TP=2 BF16 baseline, far above TP=1 BF16 (444). Full context + only 14/41 cells skipped. Best single-card option. |
